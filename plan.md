...
- **狀態**：Phase 1 已落地；Reader interaction、閱讀狀態、最近／Pin、Comments tab（查看／跳轉／修改／刪除註解與 remote branch 管理）、註解 branch export、選取文字 TTS notification controls、可選啟動 sync/rescan 已加入；Room/FTS5、WorkManager 與 Gemini query 仍在後續階段。
...
- 已建立可安裝的原生 Android app（package `com.victor.vvwiki`、app name `vv知識酷`、version `0.1.11`）。

...
- 已建立可安裝的原生 Android app（package `com.victor.vvwiki`、app name `vv知識酷`、version `0.1.12`）。
...
- Reader 顯示 repo、relative path、commit SHA、同步時間；提供頁內 Search 上／下一筆、文字選取／複製、−／＋與雙指縮放、persistent scroll、最近／Pin、comment／question highlight，固定 dark theme；compact reader chrome 只在 touch 時淡入，閒置 5 秒後淡出且不佔閱讀內容空間。
